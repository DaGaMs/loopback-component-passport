...
var debug = require(debug)("loopback-component-passport:passport-configurator");
...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  switch (authType) {
    ...
    var privateCert = options.privateCert;
    if (!cert && options.certFile) {
       cert = fs.readFileSync(options.certFile, 'utf-8');
    }
    if (!privateCert && options.keyFile) {
      privateCert = fs.readFileSync(options.keyFile, 'utf-8');
    }
    strategy = new AuthStrategy(_.defaults({
        path: options.path || '/auth/saml/callback',
        protocol: options.protocol || 'https://',
        host: options.host || 'localhost',
        entryPoint: options.entryPoint || 'https://idp.testshib.org/idp/shibboleth',
        issuer: options.issuer || 'passport-saml',
        cert: cert,
        privateCert: privateCert,
        passReqToCallback: true
      }, options),
        function(req, profile, done) {
          debug("Successfully authenticated with SAML, received profile ", profile);
          var identifier = profile.nameID; // hoping that this is generally there in a profile?
          if (link) {
            if (req.user) {
